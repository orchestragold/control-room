...
def approve(pid: int):
    ...
    # Task type is touch-specific. Touch 2/3 are approved here only when the
    # published-process branch held them for human review — normal Touch 2/3 are
    # auto-approved and scheduled directly by _generate_and_schedule_followup.
    _task_type = f'send_pitch_touch{approval.touch_number}'

    # Threading: Touch 2/3 tasks need touch1_approval_id so process_queue can
    # look up the sent_message_id and set inReplyTo / references headers.
    _touch1_approval_id = None
    if approval.touch_number > 1:
        _t1 = PitchApproval.query.filter_by(
            hubspot_contact_id=approval.hubspot_contact_id,
            company_name=approval.company_name,
            touch_number=1,
        ).first()
        if _t1:
            _touch1_approval_id = _t1.id
    ...
    db.session.add(APITaskQueue(
        ...
        payload       = {
            ...
            'touch1_approval_id': _touch1_approval_id,
    ...
    hs_write_error = None
    if approval.touch_number == 1 and approval.hubspot_contact_id:
