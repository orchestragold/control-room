# Control Room — Architecture Spec (Draft v1)
...
3. **Distribution contacts live in a spreadsheet today**, not HubSpot. Distribution gets its own pipeline built natively in Control Room's own database (mirroring Pitch Machine's stage-machine component) rather than assuming a HubSpot object — see Distribution section below.
4. **Scheduling: Google Calendar confirmed** as the date/time source of truth. Control Room owns the hold/note/notification layer on top of it.
...
8. **MailerLite:** in scope. MailerLite has a documented public REST API (campaigns, subscribers, groups, automations), so it can be connected the same way as HubSpot/Asana. Recommendation: include it, scoped to read/report status back into Control Room first (e.g. surfacing the pending A/B test winner selection) before deciding whether Control Room ever triggers sends there.
...
10. **Distribution contacts will live in HubSpot, not a native Control Room database — reversing the Session 2 assumption.** Erich's original intent was always for record stores to become real HubSpot contacts, and he confirmed sticking with that. See "CRM: build vs. buy" below for the reasoning.
...
12. **Notification digests: fully opt-in, per person.** Each person choosing to receive hold/update notifications picks their own cadence — instant, daily digest, weekly digest, or biweekly digest — rather than Control Room imposing one default for everyone.
...
### CRM: build vs. buy (HubSpot vs. a native Control Room CRM)

Erich raised whether Control Room should eventually replace HubSpot with its own lightweight CRM, mainly to escape the free-tier 1,000-record cap. Worth laying out plainly:

**Case for staying on HubSpot (recommended for now):** HubSpot already solves the unglamorous, easy-to-underestimate parts of a CRM — contact deduplication, activity history, search, list segmentation, a usable mobile app and native UI as a fallback if Control Room itself is ever down, and (for Distribution specifically) doesn't require any new build at all since it's the same object model Pitch Machine already uses. Staying on HubSpot also means one less system to secure, back up, and keep compliant with opt-out rules — that logic already needs building once, and HubSpot's contact record is a fine place to store the do-not-contact flag.
...
A CRM-pipeline view over HubSpot, not a separate database — HubSpot stays the system of record, Control Room is the working surface on top of it. Structured as pipeline stages, not a generic Trello board, so status changes carry the automation logic described below.
...
- **Underlying calendar: Google Calendar, confirmed.** Control Room wraps Google Calendar as the date/time source of truth (avoids rebuilding calendar UI and syncing headaches) and owns the hold/note/notification layer on top of it.
...
Every person (Erich, Maeve, eventually band members) sees their own upcoming tasks inside Control Room. Rather than rebuilding task management, this syncs with Asana (an MCP connector for Asana is already active in this workspace) — tasks created in Asana appear in Control Room, and tasks created in Control Room get pushed to Asana, so both stay current without picking one as the only interface.

## Tools

### Posting Tool

Working name only — "Control Room" was floated as the overall dashboard name, and the posting tool itself doesn't have a settled name yet. The June 30 OG Publisher scoping doc already covers most of the technical architecture for the first version of this, built for Orchestra Gold's channels specifically. Bringing it into Control Room means the same engine gets exposed as the shared cross-project tool, with the project/channel mapping layer added on top. Key points from that scoping session, still valid:
...
| Control Room's own database | Holds/notes/approval logs and other Control Room-native data — no longer a CRM substitute | New — not an existing system |
| Asana | Task system of record, two-way sync with Control Room's task view | MCP connector active in this workspace |
...
**Bottom line:** default to GoDaddy (Python backend + static frontend, both self-hosted there) to keep this at effectively $0 incremental cost, and treat Railway/Render/Vercel as the fallback only if resource contention with the WordPress sites turns out to be a real problem in practice — not the starting assumption. If Control Room needs its own subdomain off an existing GoDaddy domain (e.g. `control.orchestragold.com`), that's available regardless of which path is chosen — the addon-domain cap doesn't apply to subdomains.
...
**Notification fatigue — resolved.** Fully opt-in, per person: each recipient chooses their own cadence (instant, daily digest, weekly digest, or biweekly digest) rather than Control Room imposing a default.
...
**Data ownership and backup.** HubSpot, Asana, and Google Calendar (if chosen) stay systems of record, which is the right call, but it means Control Room's own database is mostly a view/cache layer plus the genuinely new data (holds, notes, approval logs). Worth being explicit about what data only exists in Control Room's own database, since that's the part that needs its own backup plan — everything else is recoverable from the source system.
...
**The GoDaddy account's SSL certificate is expired right now.** This showed up directly in the account walkthrough screenshot — "SSL Certificate: Expired," flagged by GoDaddy itself as "your domain is at risk." This is independent of Control Room entirely and worth fixing regardless of any decision above, but it's especially relevant now that this account is the leading candidate to host Control Room's backend and database, which will hold HubSpot/Meta/MailerLite credentials and business data. Worth renewing before building anything new on this account, not after.

**Login/authentication for Control Room itself.** The roles-and-permissions model (super admin, editor, member) is defined, but nothing yet specifies how someone actually logs in — email/password, Google sign-in, magic link, something else. Matters especially for band members eventually, who'll want something low-friction.

**Secrets management.** Control Room will hold live credentials for HubSpot, Meta, Google Calendar, MailerLite, and Dropbox. Needs a real answer for where those live (environment variables, a secrets manager) rather than sitting in a config file — more pointed now that the leading hosting option is shared hosting rather than a platform with built-in secrets handling.
...
**3. Database-encrypted storage.** Secrets stored in Control Room's own database, encrypted at rest, decrypted by the app when needed. *Pros:* keeps everything in one place (the app's own database) rather than a separate file or service. *Cons:* Control Room's own database backup then also contains encrypted credentials, meaning backup security matters more, and it requires building/maintaining encryption logic that Option 1 gets for free from the filesystem.
