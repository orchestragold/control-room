...
- **Progress tracking today is date properties, not a deal pipeline** — `reach_out_1` (Touch 1 sent), `reach_out_checkin` (Touch 2 sent), `reach_out_2` (Touch 3 sent — note the internal property name doesn't match its UI label of "Reach Out #3 (Close-out)," a known mismatch, not a bug). Control Room's kanban-style stage view (below) is a UI reinterpretation of this underlying date-based tracking, not a wholesale replacement of it.
